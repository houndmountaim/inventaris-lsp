      ...
      {!loading && reportData && (
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: '700' }}>Hasil Laporan</h3>
                    ...
                    {reportData.stockReport.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.item_code}>
                          ...
                          <td style={{ textAlign: 'right', fontWeight: 'bold', color: item.stock === 0 ? 'var(--danger)' : item.stock < item.min_stock ? 'var(--warning)' : 'var(--text-primary)' }}>{item.stock}</td>
                          <td>{item.unit}</td>
